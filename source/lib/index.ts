...
const hashFile = async (path, threshold = 128 * 1024) => {
    ...
    const startHash = await hashPortion(path, { start: 0, end: chunkSize });

    const midStart = Math.floor(size / 2);
    const middleHash = await hashPortion(path, {
        start: midStart,
        end: midStart + chunkSize,
    });

    const endHash = await hashPortion(path, {
